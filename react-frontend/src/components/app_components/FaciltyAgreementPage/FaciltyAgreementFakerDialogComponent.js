...
import faciltyAgreementFakerFactory from "./faciltyAgreementFakerFactory";
...
const FaciltyAgreementFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = faciltyAgreementFakerFactory(props.user,fakerCount,dealIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("faciltyAgreement").create(elem));
...
export default connect(mapState, mapDispatch)(FaciltyAgreementFakerDialogComponent);
