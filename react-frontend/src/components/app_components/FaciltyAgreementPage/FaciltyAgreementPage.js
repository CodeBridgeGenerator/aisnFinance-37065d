...
import FaciltyAgreementDatatable from "./FaciltyAgreementDataTable";
...
import FaciltyAgreementFakerDialogComponent from "./FaciltyAgreementFakerDialogComponent";
import FaciltyAgreementSeederDialogComponent from "./FaciltyAgreementSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const FaciltyAgreementPage = (props) => {
    ...
    const filename = "faciltyAgreement";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("faciltyAgreement");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Facilty Agreement", type: "error", message: error.message || "Failed get Facilty Agreement" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("faciltyAgreement").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Facilty Agreement", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("faciltyAgreement").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Facilty Agreement",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/faciltyAgreement/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Facilty Agreement </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="faciltyAgreement-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="faciltyAgreement-datatable">
                    <FaciltyAgreementDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <FaciltyAgreementEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <FaciltyAgreementCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <FaciltyAgreementFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <FaciltyAgreementSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(FaciltyAgreementPage);
