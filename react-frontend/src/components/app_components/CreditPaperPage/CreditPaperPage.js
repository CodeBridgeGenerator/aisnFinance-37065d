...
import CreditPaperDatatable from "./CreditPaperDataTable";
import CreditPaperEditDialogComponent from "./CreditPaperEditDialogComponent";
import CreditPaperCreateDialogComponent from "./CreditPaperCreateDialogComponent";
import CreditPaperFakerDialogComponent from "./CreditPaperFakerDialogComponent";
import CreditPaperSeederDialogComponent from "./CreditPaperSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const CreditPaperPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("creditPaper");
    ...
    useEffect(() => {
        ...
        client
            .service("creditPaper")
            .find({ query: { $limit: 10000 , dealId : urlParams.singleDealsId ,preparedBy : urlParams.singleProfilesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Credit Paper", type: "error", message: error.message || "Failed get Credit Paper" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("creditPaper").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Credit Paper", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("creditPaper").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/creditPaper/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="creditPaper-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="creditPaper-datatable">
                    <CreditPaperDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CreditPaperEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CreditPaperCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CreditPaperFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CreditPaperSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CreditPaperPage);
