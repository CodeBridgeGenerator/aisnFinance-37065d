...
import creditPaperFakerFactory from "./creditPaperFakerFactory";
...
client.service("profiles")
    ...
    const onRun = async () => {
        let fakeData = creditPaperFakerFactory(props.user,fakerCount,dealIdIds,preparedByIds,preparedByIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("creditPaper").create(elem));
...
export default connect(mapState, mapDispatch)(CreditPaperFakerDialogComponent);
