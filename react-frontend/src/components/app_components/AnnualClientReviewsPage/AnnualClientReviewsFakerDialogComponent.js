...
import annualClientReviewsFakerFactory from "./annualClientReviewsFakerFactory";
...
const AnnualClientReviewsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = annualClientReviewsFakerFactory(props.user,fakerCount,dealIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("annualClientReviews").create(elem));
...
export default connect(mapState, mapDispatch)(AnnualClientReviewsFakerDialogComponent);
