...
import AnnualClientReviewsDatatable from "./AnnualClientReviewsDataTable";
...
import AnnualClientReviewsFakerDialogComponent from "./AnnualClientReviewsFakerDialogComponent";
import AnnualClientReviewsSeederDialogComponent from "./AnnualClientReviewsSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const AnnualClientReviewsPage = (props) => {
    ...
    const filename = "annualClientReviews";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("annualClientReviews");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , deal : urlParams.singleDealsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Annual Client Reviews", type: "error", message: error.message || "Failed get Annual Client Reviews" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("annualClientReviews").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Annual Client Reviews", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("annualClientReviews").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Annual Client Reviews",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/annualClientReviews/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Annual Client Reviews </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="annualClientReviews-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="annualClientReviews-datatable">
                    <AnnualClientReviewsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <AnnualClientReviewsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AnnualClientReviewsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <AnnualClientReviewsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AnnualClientReviewsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AnnualClientReviewsPage);
