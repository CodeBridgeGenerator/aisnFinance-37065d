...
import businessDetailsFakerFactory from "./businessDetailsFakerFactory";
...
const BusinessDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = businessDetailsFakerFactory(props.user,fakerCount,applicationIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("businessDetails").create(elem));
...
export default connect(mapState, mapDispatch)(BusinessDetailsFakerDialogComponent);
