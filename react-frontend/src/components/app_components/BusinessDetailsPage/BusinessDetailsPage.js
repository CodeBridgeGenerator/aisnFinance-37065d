...
import BusinessDetailsDatatable from "./BusinessDetailsDataTable";
import BusinessDetailsEditDialogComponent from "./BusinessDetailsEditDialogComponent";
import BusinessDetailsCreateDialogComponent from "./BusinessDetailsCreateDialogComponent";
import BusinessDetailsFakerDialogComponent from "./BusinessDetailsFakerDialogComponent";
import BusinessDetailsSeederDialogComponent from "./BusinessDetailsSeederDialogComponent";
...
const BusinessDetailsPage = (props) => {
    ...
    const filename = "businessDetails";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("businessDetails");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Business Details", type: "error", message: error.message || "Failed get Business Details" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("businessDetails").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Business Details", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("businessDetails").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/businessDetails/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="businessDetails-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="businessDetails-datatable">
                    <BusinessDetailsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <BusinessDetailsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <BusinessDetailsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <BusinessDetailsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <BusinessDetailsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(BusinessDetailsPage);
