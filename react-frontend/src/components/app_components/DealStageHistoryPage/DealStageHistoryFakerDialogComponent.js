...
import dealStageHistoryFakerFactory from "./dealStageHistoryFakerFactory";
...
const DealStageHistoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dealStageHistoryFakerFactory(props.user,fakerCount,dealIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dealStageHistory").create(elem));
...
export default connect(mapState, mapDispatch)(DealStageHistoryFakerDialogComponent);
