...
import mortageDetailsFakerFactory from "./mortageDetailsFakerFactory";
...
const MortageDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = mortageDetailsFakerFactory(props.user,fakerCount,applicationIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("mortageDetails").create(elem));
...
export default connect(mapState, mapDispatch)(MortageDetailsFakerDialogComponent);
