...
import securityPropertiesFakerFactory from "./securityPropertiesFakerFactory";
...
const SecurityPropertiesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = securityPropertiesFakerFactory(props.user,fakerCount,applicationIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("securityProperties").create(elem));
...
export default connect(mapState, mapDispatch)(SecurityPropertiesFakerDialogComponent);
