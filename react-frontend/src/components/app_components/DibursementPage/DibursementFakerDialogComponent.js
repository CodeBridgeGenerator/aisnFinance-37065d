...
import dibursementFakerFactory from "./dibursementFakerFactory";
...
const DibursementFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [dealIdIds, setdealIdIds] = useState();
const [directorUserIdIds, setdirectorUserIdIds] = useState();
...
client.service("profiles")
                        ...
                        .then((data) => {
                        setdirectorUserIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = dibursementFakerFactory(props.user,fakerCount,dealIdIds,directorUserIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dibursement").create(elem));
...
export default connect(mapState, mapDispatch)(DibursementFakerDialogComponent);
