...
import AssetsLiabilitiesDatatable from "./AssetsLiabilitiesDataTable";
import AssetsLiabilitiesEditDialogComponent from "./AssetsLiabilitiesEditDialogComponent";
import AssetsLiabilitiesCreateDialogComponent from "./AssetsLiabilitiesCreateDialogComponent";
import AssetsLiabilitiesFakerDialogComponent from "./AssetsLiabilitiesFakerDialogComponent";
import AssetsLiabilitiesSeederDialogComponent from "./AssetsLiabilitiesSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const AssetsLiabilitiesPage = (props) => {
    ...
    const filename = "assetsLiabilities";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("assetsLiabilities");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , applicationId : urlParams.singleCommercialApplicationsId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Assets Liabilities", type: "error", message: error.message || "Failed get Assets Liabilities" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("assetsLiabilities").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Assets Liabilities", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("assetsLiabilities").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/assetsLiabilities/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="assetsLiabilities-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="assetsLiabilities-datatable">
                    <AssetsLiabilitiesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <AssetsLiabilitiesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AssetsLiabilitiesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <AssetsLiabilitiesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AssetsLiabilitiesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AssetsLiabilitiesPage);
