...
import assetsLiabilitiesFakerFactory from "./assetsLiabilitiesFakerFactory";
...
const AssetsLiabilitiesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = assetsLiabilitiesFakerFactory(props.user,fakerCount,applicationIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("assetsLiabilities").create(elem));
...
export default connect(mapState, mapDispatch)(AssetsLiabilitiesFakerDialogComponent);
