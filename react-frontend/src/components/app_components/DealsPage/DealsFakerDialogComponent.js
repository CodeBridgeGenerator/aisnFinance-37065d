...
import dealsFakerFactory from "./dealsFakerFactory";
...
const DealsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [clientIdIds, setclientIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("clients")
                        .find({ query: { $select: ["_id","clientType"] } })
                        .then((data) => {
                        setclientIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = dealsFakerFactory(props.user,fakerCount,clientIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("deals").create(elem));
...
export default connect(mapState, mapDispatch)(DealsFakerDialogComponent);
