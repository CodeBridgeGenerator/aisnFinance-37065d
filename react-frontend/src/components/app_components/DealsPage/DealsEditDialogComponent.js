...
const DealsEditDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            clientId: _entity?.clientId?._id,
status: _entity?.status,
phase: _entity?.phase,
approvedAt: _entity?.approvedAt,
rejectedAt: _entity?.rejectedAt,
rejectionReason: _entity?.rejectionReason,
        ...
        await client.service("deals").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info deals updated successfully" });
    ...
    return (
        <Dialog header="Edit Deals" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="deals-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(DealsEditDialogComponent);
