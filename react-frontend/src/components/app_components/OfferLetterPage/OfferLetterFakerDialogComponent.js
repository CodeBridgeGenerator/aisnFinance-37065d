...
import offerLetterFakerFactory from "./offerLetterFakerFactory";
...
const OfferLetterFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [dealIdIds, setdealIdIds] = useState();
const [clientProfileIds, setclientProfileIds] = useState();
const [generatedByProfileIds, setgeneratedByProfileIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("deals")
                        ...
                        .then((data) => {
                        setdealIdIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setclientProfileIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setgeneratedByProfileIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = offerLetterFakerFactory(props.user,fakerCount,dealIdIds,clientProfileIds,generatedByProfileIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("offerLetter").create(elem));
...
export default connect(mapState, mapDispatch)(OfferLetterFakerDialogComponent);
