...
import OfferLetterDatatable from "./OfferLetterDataTable";
...
import OfferLetterFakerDialogComponent from "./OfferLetterFakerDialogComponent";
import OfferLetterSeederDialogComponent from "./OfferLetterSeederDialogComponent";
...
const OfferLetterPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("offerLetter");
    ...
    useEffect(() => {
        ...
        client
            .service("offerLetter")
            .find({ query: { $limit: 10000 , dealId : urlParams.singleDealsId ,clientProfile : urlParams.singleProfilesId ,generatedByProfile : urlParams.singleProfilesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Offer Letter", type: "error", message: error.message || "Failed get Offer Letter" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("offerLetter").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Offer Letter", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("offerLetter").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/offerLetter/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="offerLetter-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="offerLetter-datatable">
                    <OfferLetterDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <OfferLetterEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <OfferLetterCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <OfferLetterFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <OfferLetterSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(OfferLetterPage);
