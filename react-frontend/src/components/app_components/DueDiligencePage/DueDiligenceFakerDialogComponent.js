...
import dueDiligenceFakerFactory from "./dueDiligenceFakerFactory";
...
client.service("profiles")
    ...
    const onRun = async () => {
        let fakeData = dueDiligenceFakerFactory(props.user,fakerCount,dealIdIds,reviewedByIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dueDiligence").create(elem));
...
export default connect(mapState, mapDispatch)(DueDiligenceFakerDialogComponent);
