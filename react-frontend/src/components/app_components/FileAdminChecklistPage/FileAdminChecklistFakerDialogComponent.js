...
import fileAdminChecklistFakerFactory from "./fileAdminChecklistFakerFactory";
...
const FileAdminChecklistFakerDialogComponent = (props) => {
...
const [applicationFormIds, setapplicationFormIds] = useState();
const [completedByProfileIds, setcompletedByProfileIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("deals")
                        .find({ query: { $select: ["_id","status"] } })
                        .then((data) => {
                        setdealIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setcheckedByIds(data.data);
                        });
client.service("commercialApplications")
                        .find({ query: { $select: ["_id","status"] } })
                        .then((data) => {
                        setapplicationFormIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setcompletedByProfileIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = fileAdminChecklistFakerFactory(props.user,fakerCount,dealIds,checkedByIds,applicationFormIds,completedByProfileIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fileAdminChecklist").create(elem));
...
export default connect(mapState, mapDispatch)(FileAdminChecklistFakerDialogComponent);
