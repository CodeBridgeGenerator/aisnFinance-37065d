...
import leadsFakerFactory from "./leadsFakerFactory";
...
const LeadsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [userIds, setuserIds] = useState();
const [assignedSalespersonIds, setassignedSalespersonIds] = useState();
...
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setassignedSalespersonIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setassignedAdminIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = leadsFakerFactory(props.user,fakerCount,userIds,assignedSalespersonIds,assignedAdminIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("leads").create(elem));
...
export default connect(mapState, mapDispatch)(LeadsFakerDialogComponent);
