...
import commercialApplicationsFakerFactory from "./commercialApplicationsFakerFactory";
...
const CommercialApplicationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = commercialApplicationsFakerFactory(props.user,fakerCount,dealIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("commercialApplications").create(elem));
...
export default connect(mapState, mapDispatch)(CommercialApplicationsFakerDialogComponent);
