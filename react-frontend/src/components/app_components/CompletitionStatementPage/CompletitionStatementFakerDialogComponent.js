...
import completitionStatementFakerFactory from "./completitionStatementFakerFactory";
...
const CompletitionStatementFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = completitionStatementFakerFactory(props.user,fakerCount,dealIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("completitionStatement").create(elem));
...
export default connect(mapState, mapDispatch)(CompletitionStatementFakerDialogComponent);
