...
import CompletitionStatementDatatable from "./CompletitionStatementDataTable";
...
import CompletitionStatementFakerDialogComponent from "./CompletitionStatementFakerDialogComponent";
import CompletitionStatementSeederDialogComponent from "./CompletitionStatementSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const CompletitionStatementPage = (props) => {
    ...
    const filename = "completitionStatement";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("completitionStatement");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Completition Statement", type: "error", message: error.message || "Failed get Completition Statement" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("completitionStatement").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Completition Statement", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("completitionStatement").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Completition Statement",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/completitionStatement/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Completition Statement </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="completitionStatement-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="completitionStatement-datatable">
                    <CompletitionStatementDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CompletitionStatementEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CompletitionStatementCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CompletitionStatementFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CompletitionStatementSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CompletitionStatementPage);
