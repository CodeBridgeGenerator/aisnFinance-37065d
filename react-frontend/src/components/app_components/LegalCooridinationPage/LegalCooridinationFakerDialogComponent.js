...
import legalCooridinationFakerFactory from "./legalCooridinationFakerFactory";
...
const LegalCooridinationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = legalCooridinationFakerFactory(props.user,fakerCount,dealIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("legalCooridination").create(elem));
...
export default connect(mapState, mapDispatch)(LegalCooridinationFakerDialogComponent);
