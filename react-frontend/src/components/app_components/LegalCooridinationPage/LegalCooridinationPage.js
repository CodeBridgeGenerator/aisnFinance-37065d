...
import LegalCooridinationDatatable from "./LegalCooridinationDataTable";
...
import LegalCooridinationFakerDialogComponent from "./LegalCooridinationFakerDialogComponent";
import LegalCooridinationSeederDialogComponent from "./LegalCooridinationSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const LegalCooridinationPage = (props) => {
    ...
    const filename = "legalCooridination";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("legalCooridination");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Legal Cooridination", type: "error", message: error.message || "Failed get Legal Cooridination" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("legalCooridination").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Legal Cooridination", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("legalCooridination").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Legal Cooridination",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/legalCooridination/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Legal Cooridination </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="legalCooridination-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="legalCooridination-datatable">
                    <LegalCooridinationDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <LegalCooridinationEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <LegalCooridinationCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <LegalCooridinationFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <LegalCooridinationSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(LegalCooridinationPage);
