...
import directorDecisionFakerFactory from "./directorDecisionFakerFactory";
...
const DirectorDecisionFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [creditpaperIdIds, setcreditpaperIdIds] = useState();
const [directorProfileIdIds, setdirectorProfileIdIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("creditPaper")
                        .find({ query: { $select: ["_id","borrowerName"] } })
                        .then((data) => {
                        setcreditpaperIdIds(data.data);
                        });
client.service("profiles")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setdirectorProfileIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = directorDecisionFakerFactory(props.user,fakerCount,creditpaperIdIds,directorProfileIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("directorDecision").create(elem));
...
export default connect(mapState, mapDispatch)(DirectorDecisionFakerDialogComponent);
