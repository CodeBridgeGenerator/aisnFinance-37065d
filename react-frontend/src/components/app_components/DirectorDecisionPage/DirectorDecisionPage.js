...
import DirectorDecisionDatatable from "./DirectorDecisionDataTable";
import DirectorDecisionEditDialogComponent from "./DirectorDecisionEditDialogComponent";
import DirectorDecisionCreateDialogComponent from "./DirectorDecisionCreateDialogComponent";
import DirectorDecisionFakerDialogComponent from "./DirectorDecisionFakerDialogComponent";
import DirectorDecisionSeederDialogComponent from "./DirectorDecisionSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const DirectorDecisionPage = (props) => {
    ...
    const filename = "directorDecision";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("directorDecision");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , creditpaperId : urlParams.singleCreditPaperId ,directorProfileId : urlParams.singleProfilesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Director Decision", type: "error", message: error.message || "Failed get Director Decision" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("directorDecision").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Director Decision", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("directorDecision").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/directorDecision/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Director Decision </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="directorDecision-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="directorDecision-datatable">
                    <DirectorDecisionDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <DirectorDecisionEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DirectorDecisionCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <DirectorDecisionFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DirectorDecisionSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(DirectorDecisionPage);
