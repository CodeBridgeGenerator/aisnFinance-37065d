...
const RicsValuationCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            dealId: _entity?.dealId?._id,instructionDate: _entity?.instructionDate,surveyorName: _entity?.surveyorName,surveyorCompany: _entity?.surveyorCompany,surveyorEmail: _entity?.surveyorEmail,surveyorPhone: _entity?.surveyorPhone,inspectionDate: _entity?.inspectionDate,valuationCost: _entity?.valuationCost,vatAmount: _entity?.vatAmount,propertyAddress: _entity?.propertyAddress,estimatedValue: _entity?.estimatedValue,propertyDescription: _entity?.propertyDescription,propertyType: _entity?.propertyType,applicantName: _entity?.applicantName,accessArrangedThrough: _entity?.accessArrangedThrough,invoiceEmail: _entity?.invoiceEmail,status: _entity?.status,
        ...
        const result = await client.service("ricsValuation").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Rics Valuation updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Rics Valuation" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Rics Valuation" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="ricsValuation-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="surveyorPhone" className="w-full mb-3 p-inputtext-sm" value={_entity?.surveyorPhone} onChange={(e) => setValByKey("surveyorPhone", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="valuationCost" className="w-full mb-3 p-inputtext-sm" value={_entity?.valuationCost} onChange={(e) => setValByKey("valuationCost", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="vatAmount" className="w-full mb-3 p-inputtext-sm" value={_entity?.vatAmount} onChange={(e) => setValByKey("vatAmount", e.value)}  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(RicsValuationCreateDialogComponent);
