...
import ricsValuationFakerFactory from "./ricsValuationFakerFactory";
...
const RicsValuationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ricsValuationFakerFactory(props.user,fakerCount,dealIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("ricsValuation").create(elem));
...
export default connect(mapState, mapDispatch)(RicsValuationFakerDialogComponent);
